  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-slate-100 font-semibold">Unified Feed</h2>
        <button
          onClick={loadFeed}
          className="hidden md:block bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white px-4 py-2 rounded-lg transition-all duration-200"
        >
          Refresh
        </button>
      </div>
              ...
              <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-2">
                    ...
                    <span className={`px-2 py-1 rounded text-xs ${getTypeColor(item.type)} whitespace-nowrap`}>
                      {item.type.charAt(0).toUpperCase() + item.type.slice(1)}
                    </span>
                    {item.type === 'task' && item.metadata.priority === 'high' && (
                      <span className="bg-red-500/20 dark:bg-red-500/20 text-red-800 dark:text-red-300 text-xs px-2 py-1 rounded whitespace-nowrap">
                        High Priority
                      </span>
                    )}
                  ...
                  <div className="flex flex-wrap items-center gap-2 md:gap-4 text-sm text-gray-500 dark:text-slate-400 mb-2">
                    {item.type === 'task' && (
                      <>
                  ...
                  {item.type === 'task' && (
                    ...
                    >
                      <button
                        ...
                        className={`px-3 py-2 rounded text-sm transition-all duration-200 cursor-pointer min-h-[44px] ${getStatusColor(item.metadata.status)} hover:opacity-80`}
                      >
